...
var enqueue = useAsap ? require('asap/raw') : IsCallable(global.setImmediate) ?
    ...
    (IsCallable(makeZeroTimeout) ? makeZeroTimeout() :
     function setTimeoutEnqueue(task) { setTimeout(task, 0); }); // Fallback
...
function triggerPromiseReaction(handler, capability, argument) {
  enqueue(function triggerPromiseReactionQueued() {
    promiseReactionJob(handler, capability, argument);
  });
}
function triggerPromiseReactions(reactions, argument) {
  _forEach(reactions, function triggerPromiseReactionsForEach(reaction) {
    enqueue(function triggerPromiseReactionsQueued() {
...
function resolvePromise(promise, resolution) {
  ...
  enqueue(function promiseResolveThenableJobQueued() {
