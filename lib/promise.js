...
var symbolSpecies = supportSpecies ? (Symbol.species || '@@species') : null;
...
var promiseResolveThenableJob = function(promise, thenable, then) {
  ...
  if (then === promisePrototypeThen) { fakeRetvalFromThen = true; }
...
defineProperties(Promise$prototype, {
  ...
  then: function then(onFulfilled, onRejected) {
    ...
    if ((!strictConstructors) && fakeRetvalFromThen) {
      resultCapability = PROMISE_FAKE_CAPABILITY;
    } else {
      C = SpeciesConstructor(promise, Promise);
      if (fakeRetvalFromThen &&
...
var optimizePromiseThen = function(promise, onFulfilled, onReject) {
  var then = promise.then;
  var result;
  if (then === promisePrototypeThen) {
    fakeRetvalFromThen = true;
    result = tryCatch2r(then, promise, onFulfilled, onReject);
    fakeRetvalFromThen = false;
    if (result === errorObj) { throw result.e; }
  } else {
...
// need the result.
Promise$prototype.then0 = function(f, r) {
...
require('./promise-extra.js')(
  Promise, PromiseCapability, optimizePromiseThen, symbolSpecies
);
